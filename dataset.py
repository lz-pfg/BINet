...
class ImageDataset(Dataset):
    ...
    def __init__(self,
        ...
        pp = path.strip().split('/')
        name = pp[-1]
        path2 = os.path.join('/../mask', name)
        self.path2 = Path(path2)
        self.name2 = Path(path2).name
        self.env2 = lmdb.open(str(path2), readonly=True, lock=False, readahead=False, meminit=False)
        with self.env2.begin(write=False) as txn2:
            self.length2 = int(txn2.get('num-samples'.encode()))
        ...
        if self.is_training and self.data_aug:
            self.augment_tfs1 = transforms.Compose([
                CVGeometry(degrees=45, translate=(0.0, 0.0), scale=(0.5, 2.), shear=(45, 15), distortion=0.5, p=0.5),
            ])
            self.augment_tfs2 = transforms.Compose([
    ...
    def get(self, idx):
        ...
        with self.env2.begin(write=False) as txn2:
            image_key, label_key = f'image-{idx + 1:09d}', f'label-{idx + 1:09d}'
            try:
                label2 = str(txn2.get(label_key.encode()), 'utf-8') 
                label2 = re.sub('[^0-9a-zA-Z]+', '', label2)
          
                label2 = label2[:self.max_length]
                
                imgbuf2 = txn2.get(image_key.encode())  
                buf2 = six.BytesIO()
                buf2.write(imgbuf2)
                buf2.seek(0)
                with warnings.catch_warnings():
                    warnings.simplefilter("ignore", UserWarning)  
                    mask = PIL.Image.open(buf2).convert("L")  
    ...
    def __getitem__(self, idx):
        image, text, idx_new, mask = self.get(idx)
        if not self.is_training: assert idx == idx_new, f'idx {idx} != idx_new {idx_new} during testing.'

        if self.is_training:
            image = self.augment_tfs2(image)
        imagenp = np.array(image)
        masknp = torch.from_numpy(np.array(mask)).unsqueeze(2).numpy()
        com = np.concatenate((imagenp, masknp), axis=2)
        com2 = self.unloader(com)

        if self.is_training:
            res = self._process_training(com2)
            image = res[:, :, 0:3]
            mask = res[:, :, 3]
        
        else:
            image = self._process_test(image)
            mask = self._process_test(mask)
